...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift in range(26):
            ...
            if numberOfValidWords > maximum:
                maximum = numberOfValidWords
                bestShift = shift
        dMessage = []
        for message in messageList:
            self.message_text = message
            result = self.apply_shift(bestShift)
            dMessage.append(result)
        
        self.message_text = ' '.join(messageList)  
        return (bestShift, ' '.join(dMessage))           
            
#Example test case (PlaintextMessage)
plaintext = PlaintextMessage('hello go', 2)
print('Expected Output: jgnnq iq')
print('Actual Output:', plaintext.get_message_text_encrypted())
    
#Example test case (CiphertextMessage)
ciphertext = CiphertextMessage('jgnnq iq')
print('Expected Output:', (24, 'hello go'))
